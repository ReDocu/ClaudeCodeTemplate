...
| 원칙 | 구현 메커니즘 |
|---|---|
| 격리 (Isolation) + 팀 내 공유 | 세션은 시작 폴더 하위만 접근. 단 **역할 세션은 공유 4폴더(`00_Project`/`11_team_doc`/`90_result_output`/`91_project_process` — 91은 읽기)에 접근** — 역할 폴더의 `.claude/settings.json`(additionalDirectories)이 허용. 타 역할·타 팀·루트는 금지 |
| 상속 (Inheritance) | CLAUDE.md가 상위→하위로 자동 로드. 루트 「전 세션 공통 규칙」이 전 세션에 적용 |
| 문서가 곧 상태 (Docs as State) | 컨텍스트는 휘발 — 팀원은 `handover_{역할}.md`에 `[NN_프로젝트명]` 태그로 기록, **팀 관리자가 process 문서(`91_project_process/`)에 취합·반영** (전담). 세션 시작 시 읽어 복원, 종료 시 갱신 |
...
│       ├── 00_Project/README.md + 01_Project01/ (작업 폴더 예시)
...
│       ├── 91_project_process/README.md + 01_Project01.md (process 문서 예시)
...
**팀 폴더 표준** (`/new_team`이 생성): 필수 5폴더(`00_Project` 공유·작업물(git 미추적) / `10_Dashboard` 관리자 전용 / `11_team_doc` 공유·문서 취합 / `90_result_output` 완료 백업·패키지 주체 / `91_project_process` 프로젝트 상태 문서·관리자 전담) + `CLAUDE.md` + `handover.md` + 역할 3공간(각 CLAUDE.md + handover + **`.claude/settings.json`** 공유 접근 허용). 번호 의미: **00 = 진행 작업, 01~0N = 역할, 10~11 = 현황·문서, 90 = 보관, 91 = 프로젝트 상태**.
...
[팀원]  0N_{역할}/handover_{역할}.md  ← [NN_프로젝트명] 태그로 기록 (역할 /handover)
              │
              ▼ 팀 관리자 /handover (팀 폴더)
[팀]    ① 91_project_process/NN_x.md 갱신 (태그 반영 + 수명주기, 전담)
        ② handover.md 취합
              │
...
| process 문서 (`91_project_process/NN_{프로젝트명}.md`) | 팀 폴더 | **팀 관리자 전담** (팀 `/handover`) | 프로젝트의 **유일한** 상태 문서 (단계·기록·블로커·수명주기) |
...
| `/new_project` | 팀 폴더 | `00_Project/`·`91_project_process/` 기존 번호 | `00_Project/NN_{이름}/`(작업 폴더) + `91_project_process/NN_{이름}.md`(process 문서) |
| `/handover` (역할) | 역할 폴더 | 이번 세션 작업 + git log | `handover_{역할}.md` (`[NN_프로젝트명]` 태그 기록) |
| `/handover` (팀) | 팀 폴더 | 역할 handover 3개 + 기존 process | **각 process 문서(`91_project_process/`) 갱신** + `handover.md` 취합 |
| `/handover_root` | 루트 | 전 팀 `handover.md` | `handover_root.md` |
| `/report` | 팀 폴더 | process 문서 전체 + 역할 handover | `11_team_doc/report_{날짜}.md` + 팀 `DASHBOARD.md` |
...
| `/compatibility_test` | 팀 폴더 | 대상 프로젝트 폴더 + `91_project_process/` + .gitignore | 호환성 점검 보고 (보정은 사용자 승인 후) |

위치 판별 규칙: 루트 = `00_Team/` 보유 · 팀 = `.../00_Team/{팀폴더}` · 역할 = 팀 직속 중 필수 5폴더 제외. **프로젝트 폴더 직접 세션은 폐지** (v1.4) — 팀원은 역할 세션의 공유 접근으로 작업한다. (팀에도 `10_Dashboard`가 있으므로 **루트 판별에 10_Dashboard를 쓰지 않는다**.)

## 6. 불변식 (Invariants) — 이것이 깨지면 비표준 상태

1. 팀 폴더명은 `ProjectTeam_{팀명}`, 필수 5폴더 + `CLAUDE.md`는 삭제·개명 금지.
2. **세션을 여는 폴더에는 CLAUDE.md + `handover_{역할}.md`가 있다** (프로젝트 폴더에는 세션을 열지 않는다 — 상태는 `91_project_process/NN_{프로젝트명}.md` 단독, 프로젝트 폴더 안 CLAUDE.md·handover.md·process.md 금지).
3. 코드 수정은 `00_Project/{프로젝트}/` 안에서만 — 주체는 개발 역할 세션(공유 접근) 또는 Builder 겸임 팀 관리자. **process 문서 갱신은 팀 관리자 전담** (팀원은 handover에 `[NN_프로젝트명]` 태그로 기록).
...
| 3 | 양식 폴더 무결성 | 양식 폴더에 필수 5폴더 + CLAUDE.md + handover.md + 역할 3공간(각 CLAUDE.md·handover·`.claude/settings.json`) + `91_project_process/01_Project01.md` |
| 4 | 각 팀 구조 준수 | 팀마다 필수 5폴더 존재, 역할 폴더에 CLAUDE.md+handover+settings.json, `00_Project/NN_*`마다 `91_project_process/NN_*.md` 존재 (프로젝트 폴더 안에는 상태 문서 없음) |
| 5 | 구식 참조 없음 | `00_result_output` · `handover_{팀명}` · `{메인폴더명}_` · `00_Project/*/process.md` 경로 검색 → PRD 변경 이력 외 0건 |
| 6 | 신선도 | 각 handover/process 제목의 `(갱신: …)` 7일 초과 → STALE, `/report_root` 권고 |
| 7 | 아카이브 후보 | process 문서(`91_project_process/NN_*.md`) 제목에 `[종료]` 검색 → 후보를 대시보드에 제안 (이동은 사용자 확인 후) |
...
| 양식 `91_project_process/01_Project01.md` | `90_Templates/process.template.md` + `/new_project` 내장본 |
...
폐지: `90_Templates/CLAUDE.project.template.md`는 v1.3에서 스텁으로 전환 (프로젝트 문서 = process 문서 단독).
...
| 버전 | 핵심 변경 |
|---|---|
| v1.6 | process 문서를 프로젝트 폴더 밖 `91_project_process/NN_{프로젝트명}.md`로 이동 (필수 5폴더), 팀 `00_Project` 전체 git 미추적 |
