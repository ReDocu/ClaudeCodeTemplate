...
rem Logic lives in `teamctl boot` (JS); this script only checks prerequisites and delegates.
...
rem Extra args pass through to teamctl boot (e.g. "start.cmd --setup" forces the wmux path prompt).
node "teamctl\bin\teamctl.js" boot %*
if errorlevel 1 (
  echo.
