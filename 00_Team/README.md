...
- **팀원 세션은 자기 역할 폴더에서 연다.** 공유 4폴더(`00_Project`/`11_team_doc`/`90_result_output`/`91_project_process` — 91은 읽기)는 역할 폴더의 `.claude/settings.json`이 접근을 허용한다. **타 역할 폴더·`10_Dashboard`는 비공유.**
- **작업물은 공유 폴더에, 기록은 내 handover에**: 프로젝트 산출물은 `00_Project/{프로젝트}/`, 기록은 `handover_{역할}.md`에 **`[NN_프로젝트명]` 태그**로.
- **process 문서(`91_project_process/NN_{프로젝트명}.md`) 갱신은 팀 관리자 전담** — 팀 폴더 `/handover`가 태그 항목을 각 process 문서에 반영하고 팀 인수인계를 취합한다.
