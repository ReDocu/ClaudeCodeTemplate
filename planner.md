...
- **문서 상태**: 🟢 Draft (v0.9) — 기능 맵·MVP 경계선 추가 + 인터랙티브 대시보드 목업 상세화
- **최종 수정**: 2026-07-09
...
| F5 | 연결 상태(Integration Health) | core(항상)+optional(선택제) 커넥터 | §6.6 | M1.6 | ◐ core만 |
...
6. 교차검증: 역할 cwd에서 `claude mcp list` 실행 → MCP 실측 대조
...
| **mcp** | 세션 MCP 서버 | `mcp-needs-auth-cache.json`로 needs-auth 판정 (§6.5 연계) |
