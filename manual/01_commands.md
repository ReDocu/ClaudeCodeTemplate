...
| `/handover` | 역할 폴더 | 역할 인수인계 갱신 (하위 프로젝트 요약 포함) | `handover_{역할}.md` |
| `/handover` | 팀 폴더 | 역할 handover 취합 | `handover.md` |
| `/handover_root` | 루트 | 전 팀 handover 취합 (요약+링크) | `handover_root.md` |
| `/report` | 팀 폴더 | 팀 정기 보고서 작성 (역할별 상세) + **팀 대시보드 갱신** | `00_result_output/report_{날짜}.md`, 팀 `10_Dashboard/DASHBOARD.md` |
...
| `/new_team {팀명}` | 루트 | 양식 폴더 복사로 팀 스캐폴딩 (역할 3공간 포함) | `ProjectTeam_{팀명}/` 전체 구조 |
| `/new_project {이름}` | 역할 폴더 | 역할 하위 프로젝트 스캐폴딩 | 프로젝트 CLAUDE.md + handover |

## 위치 규칙
- 각 명령은 실행 전 위치를 판별하고, 허용되지 않는 위치에서는 이유와 함께 거부한다.
- 루트 판별: 현재 폴더에 `00_Team/` 존재.
- 팀 폴더 판별: 경로가 `.../00_Team/{팀폴더}`로 끝남 (팀폴더 = `ProjectTeam_{팀명}`).
- 역할 폴더 판별: 경로가 `.../00_Team/{팀폴더}/{역할폴더}`. 역할폴더 = 팀 폴더 바로 아래 폴더 중 필수 폴더(`00_result_output`/`10_Dashboard`/`11_team_doc`)를 제외한 것 (표준: `01_planner`/`02_developer`/`03_package`, 커스텀 포함).
- 프로젝트 폴더 판별: 경로가 `.../00_Team/{팀폴더}/{역할폴더}/{프로젝트}`.
