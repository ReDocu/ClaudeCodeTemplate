...
| `/handover` | 팀 폴더 | **① 태그 항목을 각 process 문서(`91_project_process/`)에 반영·수명주기 갱신 ② 취합** | 갱신된 process 문서들 + `handover.md` |
...
| `/new_project {이름}` | 팀 폴더 | 프로젝트 스캐폴딩 (번호 자동 증가) | `00_Project/NN_{이름}/`(작업 폴더) + `91_project_process/NN_{이름}.md`(process 문서) |
| `/compatibility_test {이름}` | 팀 폴더 | 외부·구버전에서 가져온 프로젝트의 호환성 점검 (process 문서 계약·불변식·git 경계) — 보정은 승인 후 | 점검 보고 (+ 보정) |

## 위치 규칙
- 각 명령은 실행 전 위치를 판별하고, 허용되지 않는 위치에서는 이유와 함께 거부한다.
- 루트 판별: 현재 폴더에 `00_Team/` 존재.
- 팀 폴더 판별: 경로가 `.../00_Team/{팀폴더}`로 끝남 (팀폴더 = `ProjectTeam_{팀명}`).
- 역할 폴더 판별: 경로가 `.../00_Team/{팀폴더}/{역할폴더}`. 역할폴더 = 팀 폴더 바로 아래 폴더 중 필수 폴더(`00_Project`/`10_Dashboard`/`11_team_doc`/`90_result_output`/`91_project_process`)를 제외한 것 (표준: `01_planner`/`02_developer`/`03_package`, 커스텀 포함).
- **프로젝트 폴더 직접 세션은 폐지** (v1.4) — `00_Project/{프로젝트}`에서 `/handover` 실행 시 팀 폴더로 안내하고 중단. 팀원은 역할 세션의 공유 접근으로 작업한다.
- **팀 내 공유 접근**: 역할 세션은 공유 4폴더(`00_Project`/`11_team_doc`/`90_result_output`/`91_project_process` — 91은 읽기)에 접근 가능 — 역할 폴더의 `.claude/settings.json`이 허용.
