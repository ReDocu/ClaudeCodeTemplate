...
- **팀 내 상호 호환**: 팀원 세션은 자기 역할 폴더에서 열되, 공유 4폴더(`00_Project`/`11_team_doc`/`90_result_output`/`91_project_process` — 91은 읽기)에 접근할 수 있다 — 역할 폴더의 `.claude/settings.json`(additionalDirectories)이 허용. **타 역할 폴더·`10_Dashboard`는 비공유.**
- **프로젝트는 `00_Project/`(작업물)와 `91_project_process/`(상태)에서 팀 직속으로 관리한다.** 프로젝트마다 process 문서(`91_project_process/NN_{프로젝트명}.md`) **하나만** 둔다 — 프로젝트 폴더 안에는 상태 문서를 두지 않는다. **갱신은 팀 관리자 전담** — 팀원은 자기 handover에 `[NN_프로젝트명]` 태그로 기록하면 팀 `/handover`가 반영한다. 프로젝트 폴더 직접 세션은 없다.
- 코드 수정은 `00_Project/{프로젝트}/` 안에서만 — 주체는 개발 역할 세션 또는 팀 관리자(Builder 겸임).
- git 연결: 프로젝트 폴더는 독립 git 저장소 가능 — 템플릿 저장소는 `00_Project`를 추적하지 않고 process 문서(`91_project_process/`)만 추적한다 (루트 .gitignore).
- **커스텀 역할**: `NN_{역할}` 권장. **세션을 여는 폴더에는 `CLAUDE.md` + `handover_{역할}.md` 필수** (공유 접근 필요 시 settings.json 복사).
- 필수 폴더(00_Project/10/11/90/91)와 팀 CLAUDE.md는 삭제·개명하지 않는다.
- 작업 흐름: **기획 → 개발 → 패키지**. 역할 간 전달은 `handover_{역할}.md`(태그 포함), 프로젝트 진행은 팀 관리자가 process 문서로 통합.
...
- **팀 내 공유 예외**: 역할 세션은 자기 팀의 공유 4폴더(`00_Project`/`11_team_doc`/`90_result_output`/`91_project_process` — 91은 읽기)에 접근할 수 있다 — 역할 폴더의 `.claude/settings.json`(additionalDirectories)이 허용한다. 타 역할 폴더·`10_Dashboard`는 여전히 금지.
...
| 프로젝트 상태 문서 | `91_project_process/NN_{프로젝트명}.md` (프로젝트당 1개, 폴더명과 1:1, 팀 관리자 전담 갱신) | 91_project_process/01_login-system.md |
...
## 아카이브 규칙
- 종료 프로젝트(process 문서 제목에 `[종료]`) → `99_Archive/teams/{팀명}/{프로젝트명}/`
