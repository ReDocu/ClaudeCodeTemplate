...
2026-07-07 사용성 테스트 2회(가상 인터뷰 10인, `interview.csv`·`interview01.csv`) → 기능 백로그 `feature_changes.md`(F1~F23). **P0 전건 + F10·F11·F12 반영 완료** — 대시보드는 셸+DATA 방식(파이썬 불필요), 태그 검증·미분류 보류함, 11_team_doc 수명 폴더, §8 시간 패턴 변형까지 구현됨. 잔여는 P1(F4·F5·F6·F15·F18·F20·F22)과 M6.
git: `manager` 브랜치 (origin: github.com/ReDocu/ClaudeCodeTemplate). 커밋 `793f338`(v1.2 시점)까지 푸시됨 — **이후 작업(v1.3~v1.4·대시보드·문서 일체)은 미커밋 상태.**
...
## 진행 중인 것
- 없음 (모든 변경 작업 트리에 반영 완료 — 커밋 대기).

## 다음 할 일 (우선순위 순)
1. **미커밋 변경 커밋·푸시** — v1.3~v1.4 커밋과 v1.5(사용성 테스트+정체성 전환) 커밋 분리 제안.
2. ~~P0 잔여 3건 구현~~ → ✅ **완료 (2026-07-07)**: 대시보드 셸+DATA 전환(PRD_dashboard v1.1, 생성기 은퇴, 휴면 배지·프로젝트 일람·배너 포함) / 11_team_doc 수명 폴더(00_final/10_working/90_old — 4개 동기화 지점 반영) / handover 태그 검증+미분류 보류함 / main_manual §8 시간 패턴별 변형. **P1 잔여는 feature_changes.md 참조** (F4 이중 커밋 확인·F5·F6 커스텀 역할 자동화·F15 공용 자산 위치 결정·F18 동기화 점검·F20·F22).
3. **M6 실전 검증 (1인 시나리오로 교체)**: 1인 사용자가 도메인 팀 2~3개 + 프로젝트 다수를 2주 운영 — 명령 인식 범위·additionalDirectories 상대경로 실측, interview_priority P1 항목(#5~#12·#20) 검증 → manual/양식 보완.
...
- `10_Dashboard/dashboard.html` — **대시보드 셸 단일 원본** (DATA 블록만 명령이 교체 — 셸 수정은 제작 세션에서만) / `10_Dashboard/design/` — 디자인 예시안 3종 / 구 파이썬 생성기 — `99_Archive/2026-07-07_dashboard_generator/`
- `interview.csv` · `interview01.csv` · `interview_priority.md` · `feature_changes.md` — 사용성 테스트와 개선 백로그
