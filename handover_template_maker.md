# Handover — 템플릿 제작 (template-maker) (갱신: 2026-07-07, F36 process 문서 분리(91_project_process) 반영)
...
**F6(커스텀 역할 자동 생성) 구현 완료** — `/new_team`이 생성 전에 역할 구성(표준 3역할/커스텀)을 확인, 커스텀이면 역할별 3종 자동 생성(`90_Templates/CLAUDE.role.template.md` 신설). **구조 결정: 팀→역할∥프로젝트 현행 유지** (팀→프로젝트→역할 중첩안은 검토 후 기각 — 세션 수 폭증·v1.4 결정 3개 역행).
**F36(process 문서 분리, PRD v1.6) 구현 완료** — 프로젝트 상태 문서를 `91_project_process/NN_{프로젝트명}.md`로 이동(사용자 결정, 필수 폴더 4→5). 팀의 `00_Project`는 git 전체 미추적으로 전환. 잔여: P1(F4·F5·F15·F18·F20+F31·F22+F31·F29·F30)과 M6.
...
12. **운영 준비 3건 (2026-07-07, 사용자 요청)**: ① **F35 `/compatibility_test` 구현 완료** — 반입 프로젝트의 데이터 계약 점검 8항목+승인 보정, 명령 10종째 (PRD 7.8 /dashboard 스텁·7.9 신설, manual/01·Tech.md §5·§7-2 동기화). ② **역할 모듈 독립 활용 명시** — 표준 역할 사후 추가 = 양식 폴더에서 역할 폴더만 복사(3종 자기완결, {팀명} 치환) — new_team §4·manual/06. ③ **F33(인테이크)·F34(버전 업그레이드 호환 체계) 설계 권고 제시 후 백로그 등재** — F33은 사용자 지시로 보류, F34는 양식 폴더 소속 결정 대기.
13. **F36 — process 문서 분리 (2026-07-07, 사용자 결정, PRD v1.6)**: 프로젝트 상태 문서를 `00_Project/NN_*/process.md` → 팀 필수 폴더 **`91_project_process/NN_{프로젝트명}.md`**(파일명=프로젝트 폴더명 1:1)로 이동. **근거**: 프로젝트 폴더가 독립 git 저장소일 때 중첩 저장소 안의 process.md를 템플릿 저장소가 추적할 수 없는 기존 계약의 구조 결함 해소 + 앱 저장소에 템플릿 파일 미혼입. **부수 변경**: `.gitignore` 화이트리스트 폐지 → `00_Team/*/00_Project/` 전체 무시(양식 폴더만 예외 — check-ignore 6케이스 실측 통과), `/new_project`가 작업 폴더+process 문서 한 쌍 생성(번호는 두 폴더 최대값+1), 역할 settings.json 공유 4폴더(91은 읽기), `/compatibility_test`에 폴더 내 process.md 잔존 검사·91 이전 보정 추가, 필수 폴더 4→5(91 = 프로젝트 상태 번호대). **동기화 약 30개 파일**: 양식 폴더(팀·역할 CLAUDE/handover·91 README·00_Project README·예시 개명), 90_Templates 4종, 명령 7종(new_project 전면·handover·report·report_root·dashboard·new_team·compatibility_test), 루트 CLAUDE·PRD(v1.6)·Tech.md(§1~§8·이력)·PRD_dashboard·README·main_manual·manual 00/01/02/04/06/07·00_Team README·99_Archive README·dashboard.html 셸 제목 1건.

## 진행 중인 것
- **F34 결정 대기**: 양식 폴더(`_ProjectTeam_Template`) 소속 — 00_Team 내 유지+원칙 문서화 vs 00_Team 밖 이전(권고). 결정되면 버전 스탬프(**M6 전 필수**)·데이터 계약 명문화·업그레이드 절차 문서 착수.
...
- **process 문서 위치 = `91_project_process/NN_{프로젝트명}.md`** (F36, PRD v1.6) — 프로젝트 폴더 안에 process.md를 두는 것은 구버전 계약이다 (/compatibility_test가 잔존 검사·이전 보정). 팀의 `00_Project`는 git이 전혀 추적하지 않는다 (양식 폴더만 예외).
- **아카이브 이동은 process 문서·최종 문서만** (F27 반영됨, 99_Archive README에 명문화) — 작업물·중첩 .git을 통째로 옮기면 안 되는 이유도 거기 기록됨.
- **Tech.html은 낡음** (Tech.md 본문은 F36에서 91 반영 완료, Tech.html은 옛 구조 잔존 — F29에서 재생성 예정): 정비 전까지 Tech.html 대신 Tech.md를 본다.
- **동기화 지점** (Tech.md §8): 양식 폴더 ↔ 90_Templates 예비 사본 ↔ 명령어 내장본. 특히 process 양식은 3곳(양식 `91_project_process/01_Project01.md`/process.template/new_project 내장), 역할 settings.json은 3개 폴더 동일 내용(공유 4폴더).
...
- `feature_changes.md` — **활성 기능 백로그 (F1~F36)** — P0 2차(§E)·P1·P2 잔여와 F24 설계가 여기 있음
...
- `.claude/commands/` — 명령 10종 / `.gitignore` — 팀 `00_Project` 전체 미추적 (프로젝트 상태는 `91_project_process/`가 담당)
