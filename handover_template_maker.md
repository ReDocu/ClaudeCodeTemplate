# Handover — 템플릿 제작 (template-maker) (갱신: 2026-07-07, 3회차 가상 검증 반영)
...
**같은 날 3회차 = M6 가상 검증 수행** (시나리오 7트랙 P11~P17 + 기계 점검 1트랙): "구현 완료" 기록 중 8건 통과, **F1·F2는 유실 경로 잔존으로 재개정 필요** 판정. 신규 발견 35건 → **F25~F32** 등재 (P0 2차 4건: F26 태그 안전망 폐쇄형 / F27 완료·아카이브 핫픽스 / F25 new_team 첫 실행 경로 / F28 90_Templates 실물 드리프트). **P0 2차는 미구현 — 다음 세션 최우선.** 통합 원장: `99_Archive/2026-07-07_virtual_verification/verification_report.md`.
...
## 다음 할 일 (우선순위 순)
1. **P0 2차 구현 (3회차 발견분 — M6 시작 전)**: F26(태그 안전망 폐쇄형 — catch-all 처분·근거 보강·멱등성) → F27(완료·아카이브 핫픽스 — report_root [종료] 위치·FAQ 구모델·gitignore 충돌·백업 내용물) → F25(new_team 첫 실행 — **대괄호 폴더명 개명 여부 설계 결정 필요**) → F28(90_Templates 실물 드리프트). 상세는 `feature_changes.md` §E.
2. **M6 실전 검증 (1인 시나리오)**: 1인 사용자가 도메인 팀 2~3개 + 프로젝트 다수를 2주 운영 — 명령 인식 범위·additionalDirectories 상대경로 실측 → manual/양식 보완. **가상 검증이 넘긴 확인 항목 5건은 원장 §4 참조.**
3. **P1 잔여 구현 (M6 병행)** — `feature_changes.md` 참조: F18(동기화 점검 — **F28 수정 직후가 착수 적기**) → F29(Tech.md·Tech.html 신선도)·F30(대시보드 셸 버전 마커 등) → F4(이중 커밋 확인)·F5·F6(커스텀 역할 자동화)·F15(공용 자산 위치 — **설계 결정 선행**, V#17 타 팀 참조 경로 포함)·F20(+F31)·F22(+F31).
4. **D4**: 실제 운영 데이터로 대시보드 수용 기준(PRD_dashboard 8장) 재확인 — 프로젝트 중심 정보구조 검토 포함.
5. **F24 — Lifecycle 고정 팀** (완료 프로젝트를 라이브/보관/폐기로 분류하는 수명 관리 팀): **설계 합의됨** — 상세는 `feature_changes.md` F24. M6 이후 착수.
6. (선택) `/projects` 명령 — 팀 폴더에서 process 요약 조회 (제안만 된 상태, 미승인).
7. (보류) 각 팀 대시보드를 연결한 웹 통합 뷰 고도화 — 현재는 관리자 페이지가 그 역할.

## 주의사항 / 함정
- **정체성 = 1인 다프로젝트 운영** (PRD v1.5): 설계·우선순위 판단은 1인 시나리오를 기본 가정으로. 다인 전제 이슈는 후순위. 격리 규칙의 근거는 "권한"이 아니라 "컨텍스트 오염 방지·사고 모드 분리".
- **F27 반영 전 아카이브 이동 금지**: 현행 규칙대로 프로젝트 폴더를 99_Archive로 옮기면 .gitignore를 벗어나 작업물·중첩 .git이 템플릿 저장소에 추적된다 (3회차 V#2).
- **report_root의 [종료] 검색은 현재 오기 상태** (`report_root.md:28` — handover가 아니라 process.md 제목이 정본): F27 반영 전까지 아카이브 후보는 수동 확인.
- **Tech.md·Tech.html은 낡음** (헤더 v1.3/v0.1, 트리 어긋남 — F29): 정비 전까지 점검 기준으로 쓸 때 §2 스냅샷·§7 체크리스트 위주로만 신뢰.
