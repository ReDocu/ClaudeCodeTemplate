...
- 지시: 팀 `11_team_doc/` 내 최신 `report_{날짜}.md`가 **7일 이내**면 그것을 그대로 반환. 아니면 팀 `handover.md`·팀 대시보드(`10_Dashboard/DASHBOARD.md`)·`00_Project/*/process.md`·역할별 `handover_{역할}.md`와 `git log --since="14 days ago" -- {팀폴더}`를 근거로 양식에 맞춰 새 보고서를 작성해 반환. 파일 수정은 하지 말 것.
